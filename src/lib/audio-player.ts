...
export class AudioPlayer {
  ...
  constructor(config: ConfigManager, opts: any = {}) {
    this.autoPlay = config.get('playback.auto_play');
    this.playerInstance = player(opts);
  }
  ...
  playAudio(filePath: string): Promise<void> {
    return new Promise((resolve, reject) => {
      console.log('音声を再生中...');
      this.playerInstance.play(filePath, (err: Error | null) => {
        if (err) {
   ...
   * 自動再生が有効な場合に音声を再生する
   */
  async playIfEnabled(filePath: string): Promise<void> {
    if (this.autoPlay) {
   ...
   * 複数の音声ファイルを順番に再生する
   */
  async playMultipleFiles(filePaths: string[]): Promise<void> {
    if (!this.autoPlay) {

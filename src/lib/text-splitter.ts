...
export class TextSplitter {
   ...
   * テキストを指定した文字数で分割
   */
  splitTextByLength(text: string, maxLength: number): string[] {
    const chunks: string[] = [];
    let currentChunk = '';
   ...
   * 読み上げ用のファイルを生成（分割版）
   */
  createReadingFiles(summarizedArticles: Article[], outputDir: string): string[] {
   ...
   * 要約結果をテキストファイルに出力
   */
  outputSummaryToFile(summarizedArticles: Article[], outputDir: string, outputFileName: string): string {

...
export class VoicevoxAPI {
  ...
  async getSpeakers(): Promise<Speaker[]> {
    try {
      ...
      console.error('話者一覧の取得に失敗しました:', (error as Error).message);
   ...
   * テキストを音声に変換する
   */
  async textToSpeech(text: string, speakerId: number, outputFile: string): Promise<string> {
    try {
      // 音声合成用のクエリを生成
   ...
   * テキストを音声に変換する（再生なし）
   */
  async generateAudioFile(text: string, speakerId: number, outputFile: string): Promise<string> {
    try {
      // 音声合成用のクエリを生成
      const queryResponse = await axios.post(
        `${this.apiUrl}/audio_query?text=${encodeURIComponent(text)}&speaker=${speakerId}`,

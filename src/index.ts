 ...
 * @returns globパターンの配列
 */
function loadReadIgnorePatterns(): string[] {
  try {
    ...
    console.warn(`.readignoreファイルの読み込み中にエラーが発生しました: ${(error as Error).message}`);
  }
  return [];
}
...
async function callVoicevoxAPI(targetPath: string): Promise<void> {
 ...
 */
async function generateSingleFileAudio(
  filePath: string, 
  voicevoxAPI: VoicevoxAPI, 
  configManager: ConfigManager, 

...
import { VoicevoxAPI } from './lib/voicevox-api';
import { AudioPlayer } from './lib/audio-player';
import { FileUtils } from './lib/file-utils';
import { processUrls } from './lib/website-summarizer-core';
...
export async function main(): Promise<void> {
  ...
  try {
    ...
    const websites = (config as any).websites;
    if (!websites || !websites.urls || !Array.isArray(websites.urls) || websites.urls.length === 0) {
  ...
  "websites": {
    ...
    const urls: string[] = websites.urls;
    const outputDir: string = websites.output_dir || 'website_summaries';
    const summaryLength: string = websites.summary_length || 'medium';
 ...
 */
async function generateAndPlayAllAudio(results: ProcessResult[], configManager: ConfigManager): Promise<void> {
  try {
 ...
 */
function getTextFilesInDirectory(dirPath: string): string[] {
  try {
